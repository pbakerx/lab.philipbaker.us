...
(function () {
  ...
  const net = MW.net = {
    ...
    zone: 'lobby', status: 'off', broker: '', want: false, ws: null, tries: 0, bi: 0, buf: new Uint8Array(0), lastRx: 0, pingT: 0, retryT: 0,
    ...
    connect(zone) { net.zone = net.cleanZone(zone); net.want = true; net.tries = 0; net.bi = 0; net.open(); },
    ...
    open() {
      ...
      ws.onopen = () => { const will = JSON.stringify({ t: 'bye', id: net.id });
        ws.send(packet(0x10, cat(str('MQTT'), new Uint8Array([4, 0x06, 0, 30]), str('pbmw-' + net.id + '-' + Date.now().toString(36)), str(ROOT + net.zone + '/e'), str(will)))); };
    ...
    parse(ws, b, guard) {
      for (;;) { const buf = net.buf; if (buf.length < 2) return; let mul = 1, len = 0, i = 1, d;
        ...
        if (type === 2) { if (body[1] !== 0) { ws.close(); return; } clearTimeout(guard);
          ws.send(packet(0x82, cat(new Uint8Array([0, 1]), str(ROOT + net.zone + '/#'), new Uint8Array([0]))));
          ...
          if (!msg || typeof msg !== 'object') continue; const rest = topic.slice((ROOT + net.zone + '/').length);
    ...
    pub(topic, obj) { const ws = net.ws; if (!ws || net.status !== 'on' || ws.readyState !== 1) return false; try { ws.send(packet(0x30, cat(str(ROOT + net.zone + '/' + topic), enc.encode(JSON.stringify(obj))))); return true; } catch (e) { return false; } },
