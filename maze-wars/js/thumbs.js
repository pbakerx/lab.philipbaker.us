...
(function () {
  ...
  function tick(t) {
    ...
    // now and then, the pitch: twice a session at most (Philip: "once or twice"), eight to twelve minutes apart, never on top of another line, and only while they are actually playing
    if (t >= inviteAt && invites < 2) { if (busy || t - lastLineAt < 20000 || X.idleMs() > 30000) inviteAt = t + 15000;
