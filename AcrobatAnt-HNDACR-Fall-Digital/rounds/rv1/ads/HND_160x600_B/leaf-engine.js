...
class FallEngine {
  ...
  spawnTop(leaf, scatter) {
    Object.assign(leaf, this.baseProps(), {
      x: this.rng() * this.W,
      y: scatter ? this.rng() * this.H : -60 - this.rng() * this.H * 0.5,
    });
  }
  ...
  respawn(leaf) {
    ...
    else if (P.spawnMode === 'emitter') { const [ex, ey] = this.emitterPx(); this.spawnKicked(leaf, ex, ey); }
  ...
  drawStreaks() {
    ...
    ctx.translate(this.SW / 2 + (P.streakOffX || 0), bandMid + (P.streakOffY || 0));
    ...
    for (let i = 0; i < this.streaks.length; i++) {
      ...
      const fh = P.streakFieldH == null ? 1 : P.streakFieldH;
  ...
  streakVisMul() {
    ...
    const out = clamp(1 - (this.simT - this.streakOutAt) / d, 0, 1);
  ...
  draw() {
    ...
    if (this.sceneRect) {
      ...
      ctx.beginPath();
      if (this.sceneRadius > 0 && ctx.roundRect) ctx.roundRect(0, 0, this.SW, this.SH, this.sceneRadius);
    ...
    if (sox || soy) {
      ...
      ctx.beginPath();
      if (this.sceneRadius > 0 && ctx.roundRect) ctx.roundRect(0, 0, ew, eh, this.sceneRadius);
    ...
    if (this.cutoutOverLeaves) {
      ...
      if (this.sceneRect) {
        lc.translate(this.SX, this.SY);
        lc.beginPath();
        if (this.sceneRadius > 0 && lc.roundRect) lc.roundRect(0, 0, this.SW, this.SH, this.sceneRadius);
      ...
      if (this.sceneOffX || this.sceneOffY) {
        ...
        lc.beginPath();
        if (this.sceneRadius > 0 && lc.roundRect) lc.roundRect(0, 0, mew, meh, this.sceneRadius);
